...
<c:if test="${f:length(data.content) == 0}">
    ...
    <a class="btn btn-primary" href="/mau-sac/create">Thêm mới</a>
</c:if>
...
<c:if test="${f:length(data.content) != 0}">
   <div class="container">
       ...
       <div class="row">
           ...
           <div class="col-md-5">
               <h1 style="align-items: center">Quản lý Màu Sắc</h1>
           </div>
       </div>
       ...
       <table class="table">
           ...
           <c:forEach items="${data.content}" var="ms">
               <tr>
                   ...
                   <td>${ ms.ten }</td>
                   <td>
                       <a class="btn btn-warning" href="/mau-sac/edit/${ms.id}">Cập nhật</a>
                       <a class="btn btn-danger" href="/mau-sac/delete/${ms.id}" onclick="return confirm('Bạn có chắc chắn muốn xóa?')">Xóa</a>
                   </td>
