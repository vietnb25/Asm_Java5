...
<c:if test="${f:length(data.content) == 0}">
    ...
    <a class="btn btn-primary" href="/nhan-vien/create">Thêm mới</a>
</c:if>
<c:if test="${f:length(data.content) != 0}">
    <div class="row">
    ...
    <table class="table">
        ...
        <c:forEach items="${ data.content }" var="nv">
            <tr>
                ...
                <td>
                    <a class="btn btn-warning" href="/nhan-vien/edit/${nv.id}">Cập nhật</a>
                    <a class="btn btn-danger" href="/nhan-vien/delete/${nv.id}" onclick="return confirm('Bạn có chắc chắn muốn xóa?')">Xóa</a>
                </td>
            </tr>
        </c:forEach>
        </tbody>
